...
class Quote extends \Magento\Quote\Model\Quote
{
    ...
    {
        if ($this->quoteItem) {
            $this->quoteItemObject->setStoreId($this->quoteItem->getStoreId());
        } else {
            $this->quoteItemObject->setStoreId($this->getStoreId());
        }

        $this->quoteItemObject->setOptions($product->getCustomOptions());
        $this->quoteItemObject->setProduct($product);

        if ($request->getResetCount()
            && !$product->getStickWithinParent()
            && $this->quoteItemObject->getId() === $request->getId()
        ) {
            $this->quoteItemObject->setData(CartItemInterface::KEY_QTY, 0);
        }

        return $this->quoteItemObject;
    }
