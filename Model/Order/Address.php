...
class Address extends AbstractModel
{
    ...
    public function __construct(
        ...
        \IWD\OrderManager\Model\Order\Order $iwdOrderModel,
        ...
        $this->order = $order;
        $this->iwdOrderModel = $iwdOrderModel;
    ...
    public function updateCustomerAddress($addressData)
    {
        ...
        if ($this->customerAddress->getId()) {
            ...
            if($this->request->getParam('apply_for_customer') == 'on'){
                $order = $this->order->load( $this->request->getParam('order_id'));
                if(!is_null($order->getCustomerId())){
                    ...
                    if($this->request->getParam('billing_address') || empty($order->getBillingAddress()->getCustomerAddressId())){
                        try{$order->getBillingAddress()->setCustomerAddressId($address->getId())->save();}catch (\Exception $e){}
                    }elseif($this->request->getParam('shipping_address') || empty($order->getShippingAddress()->getCustomerAddressId())){
                        try{$order->getShippingAddress()->setCustomerAddressId($address->getId())->save();}catch (\Exception $e){}
                    }
                }
            }
        }
