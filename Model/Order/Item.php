...
class Item extends \Magento\Sales\Model\Order\Item
{
    ...
    protected function updateSkuAfterUpdateOptions()
    {
        ...
        if (strpos(Converter::getMagentoVersion(), '2.1.') !== 0){
            ...
            $options = $serializer->unserialize($productOptions);
        }
    ...
    protected function getOptionLinks($productOptions)
    {
        if (strpos(Converter::getMagentoVersion(), '2.1.') !== 0){
            ...
            $serializer = $objectManager->create(\Magento\Framework\Serialize\SerializerInterface::class);
            $productOptions = is_string($productOptions)
                ? $serializer->unserialize($productOptions)
                : $productOptions;
        }
    ...
    protected function updateQty()
    {
        ...
        if($this->scopeConfig->getValue('iwdordermanager/order_items/tier_price')){
            $this->validatePriceOfProduct($newQty,$this);
        }
    ...
    protected function validatePriceOfProduct($newQty,$item){
        ...
        if(count($tierPrice) > 0){
            ...
            foreach($tierPrice as $data){
                if($newQty >= $data['price_qty']){
                    if(isset($data['percentage_value']) && !is_null($data['percentage_value'])){
                        $realPrice = $product->getPrice() * ((100-$data['percentage_value'])/100);
            ...
            if($realPrice != $this->newParams['price']){

                $this->newParams['price'] = $realPrice;
                $this->newParams['price_incl_tax'] = $realPrice;

                $this->newParams['subtotal'] = $realPrice * $this->newParams['fact_qty'];
                $this->newParams['subtotal_incl_tax'] = $this->newParams['price_incl_tax'] * $this->newParams['fact_qty'];
                $this->newParams['row_total'] = $this->newParams['subtotal_incl_tax'];

                if(isset($this->newParams['discount_percent'])){
                    $this->newParams['discount_amount'] = ($this->newParams['subtotal']/100) * $this->newParams['discount_percent'];
                    $this->newParams['row_total'] -= $this->newParams['discount_amount'];
                }

                if(isset($this->newParams['tax_percent'])){
                    $this->newParams['tax_amount'] = ($this->newParams['row_total']/100) * $this->newParams['tax_percent'];
                    $this->newParams['price_incl_tax'] += (($realPrice/100) * $this->newParams['tax_percent']);
                }
    ...
    private function reduceProductInStock($newQty, $item)
    {
        ...
        if ($qty > 0 && $this->getAllowReturnToStock()) {
            ...
            $this->compensatingBack($item, $qty);
     ...
     * @param $backQty
     */
    private function compensatingBack($item, $backQty)
    {
        //@todo check shipment
        $shipment = $this->getOrder()->getShipmentsCollection()->getItems();
        if (interface_exists(\Magento\InventorySalesApi\Api\GetProductSalableQtyInterface::class)) {
            if (empty($shipment)) {
                $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                $reservation = $objectManager->create(\IWD\OrderManager\Model\Inventory\Reservation::class);
                $data[$item->getData('product_id')] = $backQty;
                return $reservation->execute($data, $item->getData('store_id'));
            } else {
                if ($this->scopeConfig->getValue('iwdordermanager/update_reauthorize/shipments_update_mode') == 'nothing') {

                    $websiteId = $this->getStore()->getWebsiteId();
                    $stockItem = $this->stockRegistryProvider
                        ->getStockItem($item->getProductId(), $websiteId);

                    $stockItem->setQty($stockItem->getQty() + $backQty);
                    $stockItem->save();
                }
        ...
        return $this->productToInventory($backQty, $item->getProductId());
    }
    ...
    protected function productToInventory($qty, $productId)
    {
        ...
        $this->stockIndexer->reindexList([$productId]);
        $this->priceIndexer->reindexList([$productId]);
    }
    ...
    private function increase($item, $qty)
    {
        $shipment = $this->getOrder()->getShipmentsCollection()->getItems();
        if (interface_exists(\Magento\InventorySalesApi\Api\GetProductSalableQtyInterface::class)) {
            if (empty($shipment) || $this->scopeConfig->getValue('iwdordermanager/update_reauthorize/shipments_update_mode') == 'nothing') {
                $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                $reservation = $objectManager->create(\IWD\OrderManager\Model\Inventory\Reservation::class);
                $data[$item->getData('product_id')] = -$qty;
                return $reservation->execute($data, $item->getData('store_id'));
            }
            return;
        }

        return $this->productFromInventory($qty, $item->getProductId());
    }
